program melting
    ...
    integer i, j, k
    integer,parameter :: imax=61, jmax=61, kmax=61
    integer count_w
    ...
    call fieldview(1, 0, T(:,:,:), beta(:,:,:), imax, jmax, kmax, dx)
    ...
    do n = 1, nmax
        ...
        if(mod(n, 500) .eq. 0 .and. n .le. 3000) then
            call fieldview(2, n, T(:,:,:), beta(:,:,:), imax, jmax, kmax, dx)
        endif
...
!Fieldview用の可視化ファイルを出力するサブルーチン
subroutine fieldview(mode, step, T, beta, imax, jmax, kmax, dx)
    integer mode
    integer step
    integer i, j, k
    integer imax, jmax, kmax
    ...
    if(mode .eq. 1) then
        ...
        open(11, file='GRID.P3D', status='unknown', blank='null')
