...
C------------------------------------------------------------
      ...
      common /temperature/ T(if,jf,kf)
      common p(if,jf,kf), u(if,jf,kf), v(if,jf,kf), w(if,jf,kf)
      real :: dx, dy, dz, dx2, dy2, dz2, c0, omega
...
c--------- グリッドデータ出力 ---------
      ...
      enddo
c--------- グリッドデータ書込 ---------
      open(11,file='grid_sample.DAT',status='unknown', blank='null')
      ...
      close(11)
c---------- 結果データ出力 ---------
      open(11,file='result_sample.DAT',status='unknown', blank='null')
       uin=1.0
       vis=0.0
       tin=0.0
       time=0.0
      ...
      close(11)
c---------- 流速データ出力 ---------
      !open(11, file='velocity_sample.DAT', status='unknown', blank='null')
       !write(11, 111) if, jf, kf
       !do k = 1, kf
         !do j = 1, jf
           !do i = 1, if
             !write(11, 112) u(i, j, k), v(i, j, k), w(i, j, k)
           !end do
         !end do
       !end do
      !close(11)
